...
BusInOut switches(BTN1_PIN, BTN2_PIN, BTN3_PIN, BTN4_PIN);

// Outputs
// DigitalOut ledRed(TRAF_RED1_PIN);
// DigitalOut ledYel(TRAF_YEL1_PIN);
// DigitalOut ledGrn(TRAF_GRN1_PIN);
BusOut led(TRAF_GRN1_PIN, TRAF_RED1_PIN, TRAF_YEL1_PIN);

int main()
{
    switches.input();          // configure as inputs
    switches.mode(PullDown);   // set pull-downs
    
    while (true) {

        int sw = switches.read();
